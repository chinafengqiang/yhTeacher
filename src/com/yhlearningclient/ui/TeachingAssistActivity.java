 ...
 * 教辅资料
 */
public class TeachingAssistActivity extends Activity {
	...
	private final String pathName = "myCourseBook";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tab_teaching_assist);
	...
	private void loadData(int categoryId){
		...
		try{
			...
			Toast.makeText(TeachingAssistActivity.this, "网络连接出现问题，请检查！", Toast.LENGTH_LONG).show();
		}
	...
	private View.OnClickListener onClickListenerView = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.main_bottom_download1:
				Intent intent3 = new Intent(context, MyCourseBookActivity.class);
	...
	private Handler handleCourses= new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			if (books == null) {
				Toast.makeText(TeachingAssistActivity.this, "对不起，教辅资料还没公布", Toast.LENGTH_SHORT).show();
			}else{
 ...
 class CourseTableList implements Runnable{
	   ...
	   private int categoryId; 
		...
		public void run() {
	    	BookManager manager = new BookManager();
				try {
					bookList = manager.geBookByPage(this.serverIp,Integer.parseInt(classId.toString()),1, 12000);
				} catch (Exception e) {
				}
	...
	//得到当前下载的文件名
		...
		private void downloadVideos(final String url){
			...
			if(FileUtil.isExists("/sdcard/myCourseBook/" +fileName)){
			     ...
			     .setPositiveButton("确定", new DialogInterface.OnClickListener(){
					...
					public void onClick(DialogInterface dialog, int which) {
						DownFileTask task = new DownFileTask(context,handler,fileName,"myCourseBook");
						task.execute(url);
					}
