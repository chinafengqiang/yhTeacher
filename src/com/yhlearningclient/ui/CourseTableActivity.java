...
public class CourseTableActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tab_course_table);
	...
	private void loadData(){
		...
		try{
			...
			Toast.makeText(CourseTableActivity.this, "网络连接出现问题，请检查！", Toast.LENGTH_LONG).show();
		}
	...
	private Handler handleCourses= new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			if (coursePlans == null) {
				Toast.makeText(CourseTableActivity.this, "对不起，这学期课程表还没公布", Toast.LENGTH_SHORT).show();
			}else{
 ...
 class CourseTableList implements Runnable{
	   ...
	   private String serverIp = "";
		...
		public void run() {
			AdviodManager manager = new AdviodManager();
				try {
					coursePlanList = manager.getCourseTableByAll(this.serverIp);
				} catch (Exception e) {
				}
	...
	//得到当前下载的文件名
		...
		private void downloadVideos(final String url){
			...
			if(FileUtil.isExists("/sdcard/myCourseTable/" +fileName)){
				...
				DownFileTask task = new DownFileTask(context,handler,fileName, "myCourseTable");
				task.execute(url);
