...
public class TestPaperTreeActivity extends Activity implements OnUploadProcessListener{
	...
	Long classId;
	private String[] is = { "下载试题", "下载学员作业","批改学员作业","取消" };
			...
			builder.setItems(is, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					case 0:
						...
						Integer testPaperId1 = Integer.parseInt(element1.getId().toString());
						String serverIp = "http://"+ ip +":8082";
						loadTestPaperList(serverIp, testPaperId1);
...
//						}
					...
					case 1:
						...
						Integer testPaperId2 = Integer.parseInt(element2.getId().toString());
						String serverIp1 = "http://"+ ip +":8080";
						loadUserTestPaperList(serverIp1, testPaperId2);
						break;
					case 2:
						...
						Intent intent3 = new Intent(context, ContactActivity.class);
						intent3.putExtra("categoryId", ids);
	...
	//用于接受loadUserTestPaperList线程结束后传来的列表，并加以显示
		Handler handlerTestPaperRecord = new Handler() {
			public void handleMessage(android.os.Message msg) {
				...
				if(flag){
					...
					Toast.makeText(TestPaperTreeActivity.this, "下载试卷成功", Toast.LENGTH_SHORT).show();
					pd.setMessage("数据获取中,请稍候...");
					hideProgressDialog();
				} else {
					Toast.makeText(TestPaperTreeActivity.this, "下载试卷失败", Toast.LENGTH_SHORT).show();
				}
		...
		//用于接受loadUserTestPaperList线程结束后传来的列表，并加以显示
		Handler handlerUserTestPaperRecord = new Handler() {
			public void handleMessage(android.os.Message msg) {
				boolean flag = (Boolean) msg.obj;
				if(flag){
					pd.setMessage("数据已获取,界面绑定中...");
					Toast.makeText(TestPaperTreeActivity.this, "下载学员作业成功", Toast.LENGTH_SHORT).show();
					pd.setMessage("数据获取中,请稍候...");
					hideProgressDialog();
				} else {
					Toast.makeText(TestPaperTreeActivity.this, "下载学员作业失败", Toast.LENGTH_SHORT).show();
				}
	...
	//创建新线程从本地试卷列表
	public void loadTestPaperList(final String serverIP, final int testPaperId) {
		
		showProgressDialog();
		Thread thread = new Thread() {
			@Override
			public void run() {
				boolean  flag = TestPaperFactory.createTestPaper(TestPaperTreeActivity.this).isTestPaperQuestionList(serverIP, testPaperId);
	...
	//创建新线程从本地试卷列表
	public void loadUserTestPaperList(final String serverIP, final int testPaperId) {
		
		showProgressDialog();
		Thread thread = new Thread() {
			@Override
			public void run() {
				boolean  flag = UTestPaperFactory.createUTestPaper(TestPaperTreeActivity.this).isUserTestPaperList(serverIP, testPaperId);
				Message message = new Message();
				message.obj = flag;
				handlerUserTestPaperRecord.sendMessage(message);
			}
		};
