...
public class TeachingMessageActivity extends Activity {
	...
	private void bindClickListener() {
		...
		inform_list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				userManager.updataSysMessage(sysMessage);
		...
		addInform.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intent2 = new Intent(context, addMessageActivity.class);
	...
	private Handler handleNewSysMessages = new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			List<SysMessage> list = (ArrayList<SysMessage>)msg.obj;
			if (list == null) {
				Toast.makeText(TeachingMessageActivity.this, "对不起，无最新教学通知", Toast.LENGTH_SHORT).show();
			}else{
				pd.setMessage("数据已获取,界面绑定中...");
				
				adapter = new MyAdapter(list, TeachingMessageActivity.this);
				inform_list.setAdapter(adapter);
				pd.setMessage("数据获取中,请稍候...");
				hideProgressDialog();
			}	
	...
	class LoadMessageList implements Runnable{
		...
		public void run() {
			...
			try{
				sysMessages = userManager.getAllSysMessageList(serverIp, classId);
				last_message_id = userManager.getLastSysMessageId();
	...
	class LoadNewMessageList implements Runnable{
		...
		public void run() {
			try{
				messages = userManager.getRefreshSysMessageList(serverIp,classId);
			} catch(Exception e){
			}
			Message message = new Message();
