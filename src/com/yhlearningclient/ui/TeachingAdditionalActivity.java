 ...
 * 补充资料
 */
public class TeachingAdditionalActivity extends Activity {
...
//	private Button bottom_category = null;
	private final String pathName = "myAdditionalFile";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tab_additional);
...
//		bottom_category.setOnClickListener(onClickListenerView);
	...
	private void loadData(int categoryId){
		...
		try{
			...
			Toast.makeText(TeachingAdditionalActivity.this, "网络连接出现问题，请检查！", Toast.LENGTH_LONG).show();
		}
	...
	private View.OnClickListener onClickListenerView = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.main_bottom_download1:
				Intent intent3 = new Intent(context, MyAdditionalFileActivity.class);
...
//				break;
	...
	private Handler handleCourses= new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			if (books == null) {
				Toast.makeText(TeachingAdditionalActivity.this, "对不起，补充资料还没公布", Toast.LENGTH_SHORT).show();
			}else{
				...
				//	String url = book.getUrl();
					...
					Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.additionalfile);
 ...
 class CourseTableList implements Runnable{
	   ...
	   private int categoryId; 
		...
		public void run() {
	    	BookManager manager = new BookManager();
				try {
					bookList = manager.getAdditionalall(this.serverIp, Integer.parseInt(classId.toString()), 1, 12000);
				} catch (Exception e) {
				}
	...
	//得到当前下载的文件名
		...
		//	Tool.ShowMessage(context, fileName);
			if(FileUtil.isExists("/sdcard/myAdditionalFile/" +fileName)){
			     ...
			     .setPositiveButton("确定", new DialogInterface.OnClickListener(){
					...
					public void onClick(DialogInterface dialog, int which) {
						DownFileTask task = new DownFileTask(context,handler,fileName,"myAdditionalFile");
						task.execute(url);
					}
